...
//        layer.transform = scaleTransform
    ...
    func getCurrentAnimationScale() -> Double{
        var scale : CGFloat = 1
        var width : CGFloat
        if let temp = self.layer.presentationLayer()?.valueForKeyPath("transform.scale"){
            scale = CGFloat(temp as! NSNumber)
        } else{
            scale = CGFloat(self.layer.valueForKeyPath("transform.scale") as! NSNumber)
    ...
    func startTakingPictureAnimations(expandDur: Double, shrinkDur: Double){
        ...
        //shrinkCircleView(expandDur, to: 1.2)
        self.rotateAndShrinkCircleView(expandDur, rotateDuration: 5.3, shrinkDuration: shrinkDur)

    }
    
    func printBounds() {
        if let preslayer = self.layer.presentationLayer(){

            print("bounds: \(preslayer.bounds)")
            print("frame: \(preslayer.frame)")
            print("origin: \(preslayer.anchorPoint)")
            
            print("bounds: \(layer.bounds)")
            print("frame: \(layer.frame)")
            print("origin: \(layer.anchorPoint)")
